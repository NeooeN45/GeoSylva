...
object DateTimeFormatter {
    ...
    /** Format : 30/06/2026 */
    fun date(date: Date = Date()): String = DATE_FR.get().format(date)

    /** Format : 30/06/2026 14:30 */
    fun dateTime(date: Date = Date()): String = DATETIME_FR.get().format(date)

    /** Format : 2026-06-30 (ISO) */
    fun dateIso(date: Date = Date()): String = DATE_ISO.get().format(date)

    /** Format : 20260630-1430 (pour noms de fichiers) */
    fun timestampFile(date: Date = Date()): String = TIMESTAMP_FILE.get().format(date)

    /** Format : 06/26 (mois/année court) */
    fun monthYear(date: Date = Date()): String = MONTH_YEAR.get().format(date)

    /** Format : 06/26 (mois/année court) */
    fun monthYear(dateMs: Long): String = MONTH_YEAR.get().format(Date(dateMs))

    // ── Formatage depuis timestamp (Long) ─────────────────────────────────────

    fun date(dateMs: Long): String = DATE_FR.get().format(Date(dateMs))

    fun dateTime(dateMs: Long): String = DATETIME_FR.get().format(Date(dateMs))

    fun dateIso(dateMs: Long): String = DATE_ISO.get().format(Date(dateMs))
