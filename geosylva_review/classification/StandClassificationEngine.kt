...
object StandClassificationEngine {
    ...
    private fun detectComposition(stats: MartelageStats): StandComposition {
        ...
        return when {
            ...
            // Sans données GPS individuelles, on ne peut pas distinguer bouquets vs pied-à-pied
    ...
    private fun detectDisturbance(stats: MartelageStats): DisturbanceState {
        ...
        val hasSlenderness = stats.dg != null && stats.meanH != null && stats.dg > 0 &&
                             (stats.meanH / stats.dg) > 1.2

        return when {
            dyingPct > 20 || (deadPct > 15 && bio?.deadTreeCount?.let { it > 5 } == true) ->
                DisturbanceState.DEPERISSANT
            dyingPct > 8 || hasErrors -> DisturbanceState.STRESSE
            hasSlenderness -> DisturbanceState.STRESSE
            deadPct > 5 -> DisturbanceState.PERTURBE
